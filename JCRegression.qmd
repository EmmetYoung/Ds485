...
jc_goals_model3 <- lm(jc_goals ~ wide_attacks_total +  free_kicks_successful + box_touches + attacking_clear_cuts +  won_off_backline + jc_shots + opponent_shots  + massey_ranking + second_ball_won + backline_to_frontline , data = soccer)
...
jc_chances_model3 <- lm(attacking_clear_cuts ~ defensive_wide_attacks_total+ backline_to_frontline  + free_kicks_successful + won_off_backline + jc_shots + massey_ranking, data = soccer)
...
tibble(massey_ranking = soccer_logistic$massey_ranking,
  ...
  ggplot()+ geom_point(aes(massey_ranking, type)) + geom_line(aes(massey_ranking, preds), color = "deeppink")+ theme_bw()
```
...
#| warning: false
#| message: false
rank_model1 <- train(train_data[, -1, -36, -37],
                    train_data$result,
                    method = "rf",
...
#| warning: false
#| message: false
rank_model <- train(train_data[, -1, -36, -37],
                    train_data$result,
                    method = "rpart",
                    trControl = trainControl(method="cv"))
```
...
ggplot(soccer_logistic) + geom_point(aes(opponent_clear_cuts,attacking_clear_cuts, color = result)) + 
    ...
    labs(x = "Opponent Clear Cuts", y = "Attacking Clear Cuts", title = "Opponent Chances vs Juniata Chances") +
  theme_bw()
```
